...
def benchmark_sweep(
    ...
    granularity: int = 100, beta: float = 1.0, repeats: int = 3,
) -> Dict[str, Any]:
    ...
    print(f"  {label:<32s} | steps={sweep_length:>8d} | "
          ...
          f"{steps_per_sec:>8.0f} steps/s | peak_mem={mem_str}")
...
def benchmark_replica_scaling(
    ...
    sweeps: int = 50, granularity: int = 200, repeats: int = 3,
) -> List[Dict[str, Any]]:
    ...
    for R in replicas_grid:
        ...
        t_std = statistics.stdev(times) if len(times) > 1 else 0.0
        steps_per_s = sweep_length / t_mean
        peak_mb = _gpu_peak_mb()
        ...
        print(f"  R={R:>5d} | N={N:>4d} | trace={t_trace:6.2f}s | "
              ...
              f"{steps_per_s:>9.0f} steps/s | peak_mem={mem_str}")

        results.append({
            "label": label, "replicas": R, "N": N,
            "t_trace": t_trace, "t_mean": t_mean, "t_std": t_std,
            "steps_per_s": steps_per_s, "peak_mem_mb": peak_mb,
        })
...
def benchmark_overlap_scaling(replicas_grid: List[int], n_spins: int = 64, repeats: int = 5) -> List[Dict[str, Any]]:
...
def plot_lattice_scaling(lattice_results: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    for color, (label, rows) in zip(colors, lattice_results.items()):
        ...
        ax.plot(Ns, rates, 'o-', color=color, markersize=5, linewidth=1.5, label=label)
...
def plot_replica_scaling(model_results: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    for color, (label, rows) in zip(colors, model_results.items()):
        ...
        if reps:
            axs[0].plot(reps, rates, 'o-', color=color, markersize=5, linewidth=1.5, label=label)
...
def main():
    ...
    for L in [8, 16, 32]:
        ...
        r = benchmark_sweep(f"Ising 2D L={L} N={N}", system,
                             [Energy(system), Magnetization(system)], sweep_length, granularity)
    ...
    for L in [4, 6, 8]:
        ...
        r = benchmark_sweep(f"EA 3D L={L} N={N}", system,
                             [Energy(system), OverlapDistribution(system)], sweep_length, granularity)
    ...
    spherical_result = benchmark_sweep(f"Spherical 2D L={L} N={N}", system,
                                        [Energy(system)], sweep_length, granularity)
    ...
    print("-" * 96)
    overlap_results = benchmark_overlap_scaling([16, 64, 256, 1024, 4096, 16384])
    ...
    for label, make_meas in [
        ...
        r = benchmark_sweep(f"EA3D L=4 [{label}]", system, make_meas(system), sweep_length, granularity)
    ...
    for gran in [50, 100, 500, 1000]:
        ...
        r = benchmark_sweep(f"EA3D L=4 gran={gran} ({num_recordings} recs)",
                             system, [Energy(system), OverlapDistribution(system)], sweep_length, gran)
